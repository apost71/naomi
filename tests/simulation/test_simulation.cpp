...
TEST(TestSimulation, TestAttitude)
{
  ...
  std::shared_ptr<attitude_provider> torque_free_attitude = std::make_shared<attitude::torque_free_attitude>(initial_attitude, geom.get_inertia_tensor());
  std::shared_ptr<spacecraft> sc = std::make_shared<spacecraft>("test", state_vec, 100.0, torque_free_attitude);
